...
const componentItGen = (key: string, childFreqKey: string) => {
  ...
  // })
  // `;
  return `expect(wrapper.find(${key}).length).toBe(${childFreqKey})`;
};
...
const htmlCountFindGen = (key: string, htmlFreqKey: string) => {
	return `expect(wrapper.find('${key}').length).toEqual(${htmlFreqKey});`;
};
...
const htmlInnertextFindGen = (elementType: string, innerText: string) => {
	return `expect(wrapper.find('${elementType}').text()).toEqual(${innerText});`;
};
